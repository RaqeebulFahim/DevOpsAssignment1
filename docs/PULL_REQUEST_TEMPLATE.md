<!--
  Place this file in your repository at:
    - .github/PULL_REQUEST_TEMPLATE.md   (recommended)
    - or the repository root / docs/

  GitHub will auto-fill new PRs with this template.
  Students: copy this into your own repo and adapt it.
-->

## 📝 Description

Please include a summary of the change and which issue (if any) is fixed.
Also include relevant motivation and context.

Fixes # (issue number, if applicable)

---

## 🔧 Type of change

Please delete options that are not relevant.

- [ ] 🐛 Bug fix (non-breaking change which fixes an issue)
- [ ] ✨ New feature (non-breaking change which adds functionality)
- [ ] 💥 Breaking change (fix or feature that would cause existing functionality to not work as expected)
- [ ] 📚 Documentation update
- [ ] ♻️ Refactor / code cleanup

---

## ✅ Checklist

- [ ] My code follows the style guidelines of this project
- [ ] I have performed a self-review of my own code
- [ ] I have commented my code, particularly in hard-to-understand areas
- [ ] I have made corresponding changes to the documentation
- [ ] My changes generate no new warnings
- [ ] I have added tests that prove my fix is effective or my feature works
- [ ] New and existing unit tests pass locally with my changes
- [ ] Any dependent changes have been merged and published in downstream modules
- [ ] Commit messages are descriptive and meaningful
- [ ] Branch name is meaningful (e.g., `feature/login`, `dev`, `bugfix/typo`)

---

## 📸 Screenshots / Outputs (if applicable)

| Before | After |
| ------ | ----- |
|        |       |

---

## 🧪 How Has This Been Tested?

Please describe the tests that you ran to verify your changes. Provide
instructions so we can reproduce.

1.
2.
3.

**Test configuration:**
- OS:
- Browser / Runtime:
- Version:

---

## 💬 Additional context

Add any other context, links, or references about the pull request here.